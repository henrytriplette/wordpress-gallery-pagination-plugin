<?php
/*
Plugin Name: Triplette Paginated Gallery
Plugin URI: http://www.triplette.it
Description: Adds pagination to standard wordpress gallery
Version: 0.1.1
...
/*  Copyright 2014  Henry Triplette  (email : scrivi@triplette.it)
...
if (get_option('use_gallery_shortcode')) {
	...
	add_shortcode('gallery', 'triplette_paginated_gallery');
} else {
	add_shortcode('paginated-gallery', 'triplette_paginated_gallery');
}

function triplette_paginated_gallery($attr) {
	$post = get_post();
	...
	if ($pageCount > 1) {
		...
		$gplist= '<div class="gallery_pages_list">'.__('Gallery Page').'<hr /> <ul class="pagination pagination-sm">';
		for ( $j=1; $j<= $pageCount; $j++)
		{
			if ( $j==$currentPage ) {
				if ( $j < 10 ) { $j2 = '0'.$j; } else { $j2 = $j; }
				$gplist .= '<li class="active"><a href="#">'.$j2.' </a></li>';
			} else {
				if ( $j < 10 ) { $j2 = '0'.$j; } else { $j2 = $j; }
				$gplist .= '<li><a href="'.$page_link. ( ($page_link_perma?'?':'&amp;') ). 'galleryPage='.$j.'">'.$j2.'</a></li> ';
			}
		}
	...
	// $output .= 'Gallery Here - Page '. $currentPage. ' of '. $pageCount;
